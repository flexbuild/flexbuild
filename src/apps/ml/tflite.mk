...
tflite:
	@[ $(DESTARCH) != arm64 -o $(DISTROVARIANT) = tiny -o $(DISTROVARIANT) = base ] && exit || \
	 [ `hostname` != fbdebian ] && $(call fbprint_w,"Please run bld docker to build tflite in docker") && exit || \
	 ...
	 cp $(FBDIR)/patch/tflite/tensorflow2-lite.pc $(DESTDIR)/usr/share/pkgconfig && \
	 \
	 $(call fbprint_n,"install examples") && \
	 install -d $(DESTDIR)/usr/bin/tensorflow-lite-2.14.0/examples && \
	 ...
	 install -m 0555 examples/label_image/label_image $(DESTDIR)/usr/bin/tensorflow-lite-2.14.0/examples && \
	 install -m 0555 tools/benchmark/benchmark_model $(DESTDIR)/usr/bin/tensorflow-lite-2.14.0/examples && \
	 install -m 0555 tools/evaluation/{coco_object_detection_run_eval,imagenet_image_classification_run_eval,inference_diff_run_eval} \
		 $(DESTDIR)/usr/bin/tensorflow-lite-2.14.0/examples && \
	 \
	 $(call fbprint_n,"install label_image data") && \
	 cp $(MLDIR)/tflite/tensorflow/lite/examples/label_image/testdata/grace_hopper.bmp $(DESTDIR)/usr/bin/tensorflow-lite-2.14.0/examples && \
	 cp $(MLDIR)/tflite/tensorflow/lite/java/ovic/src/testdata/labels.txt $(DESTDIR)/usr/bin/tensorflow-lite-2.14.0/examples && \
	 \
	 $(call fbprint_n,"install mobilenet tflite file python example and pip package") && \
	 cp $(MLDIR)/tflite/mobilenet_*.tflite $(DESTDIR)/usr/bin/tensorflow-lite-2.14.0/examples && \
	 cp $(MLDIR)/tflite/tensorflow/lite/examples/python/label_image.py $(DESTDIR)/usr/bin/tensorflow-lite-2.14.0/examples && \
